...
function createAudioModal(base64, focusId) {
  if (!isValidBase64(base64)) {
    log('error', 'Invalid base64 format for audio modal');
    showStatusMessage('Invalid audio data received.');
    return;
  }
...
function base64ToArrayBuffer(base64) {
  if (!isValidBase64(base64)) {
    log('error', 'Invalid base64 string in base64ToArrayBuffer');
...
async function importPublicKey(base64) {
  if (!isValidBase64(base64)) {
    log('error', 'Invalid base64 format for public key');
...
async function decrypt(encrypted, iv, salt, master) {
  if (!isValidBase64(encrypted) || !isValidBase64(iv) || !isValidBase64(salt)) {
    log('error', 'Invalid base64 format in decrypt inputs');
    throw new Error('Invalid base64 format in decrypt inputs');
  }
  const hkdfKey = await window.crypto.subtle.importKey(
    'raw',
    master,
...
async function decryptBytes(key, encrypted, iv) {
  if (!isValidBase64(encrypted) || !isValidBase64(iv)) {
    log('error', 'Invalid base64 format in decryptBytes inputs');
    throw new Error('Invalid base64 format in decryptBytes inputs');
  }
...
async function verifyMessage(signingKey, signature, data) {
  if (!isValidBase64(signature)) {
    log('error', 'Invalid base64 format for signature');
    throw new Error('Invalid base64 format for signature');
  }
  const encoded = new TextEncoder().encode(data);
