  ...
  try {
    ...
    if ((message.type === 'message' || message.type === 'image' || message.type === 'voice') && useRelay) {
      ...
      try {
        ...
        console.error('Decryption error in relay mode:', error);
        showStatusMessage('Failed to decrypt relay message.');
        return;
      }
      const senderUsername = parsed.username;
      const timestamp = parsed.timestamp || Date.now(); // Fallback if missing
      ...
      if (message.type === 'image') {
        ...
        img.addEventListener('click', () => createImageModal(parsed.data, 'messageInput'));
        messageDiv.appendChild(img);
      } else if (message.type === 'voice') {
        const audio = document.createElement('audio');
        audio.src = parsed.data;
        audio.controls = true;
        audio.setAttribute('alt', 'Received voice message');
        audio.addEventListener('click', () => createAudioModal(parsed.data, 'messageInput'));
        messageDiv.appendChild(audio);
      } else {
        messageDiv.appendChild(document.createTextNode(sanitizeMessage(parsed.content)));
      }
  ...
  navigator.mediaDevices.getUserMedia({ audio: true })
    .then(stream => {
      if (!Recorder.isRecordingSupported()) {
        showStatusMessage('Error: Opus recording not supported in this browser.');
        return;
      }
      ...
      mediaRecorder.onstop = async () => {
        const buffer = new Uint8Array(chunks.reduce((acc, arr) => acc.concat(Array.from(arr)), []));
